...
use editor::{EditorElement, EditorStyle, actions::OpenExcerpts};
...
pub(crate) enum HistoryFilterKind {
    UserMessages,
    AssistantMessages,
    Thinking,
    ToolCalls,
...
impl PermissionTextPrompt {
    fn from_meta(meta: Option<&acp::Meta>) -> Option<Self> {
        let prompt = meta?.get(TEXT_PROMPT_META_KEY)?.as_object()?;
        let mode = match prompt.get("mode").and_then(|value| value.as_str()) {
...
fn merge_permission_outcome_meta(
...
#[cfg(test)]
mod permission_text_prompt_tests {
    use super::*;

    #[test]
    fn parses_single_line_text_prompt_meta() {
    ...
    #[test]
    fn parses_multiline_text_prompt_meta() {
        let meta = acp::Meta::from_iter([(
            TEXT_PROMPT_META_KEY.into(),
            serde_json::json!({
                "mode": "editor",
...
pub struct ThreadView {
    ...
    permission_text_editors: RefCell<HashMap<acp::ToolCallId, Entity<Editor>>>,
    permission_text_editor_subscriptions: RefCell<HashMap<acp::ToolCallId, Subscription>>,
...
impl ThreadView {
    ...
    pub(crate) fn new(
        ...
        let session_id = thread.read(cx).session_id().clone();
        let parent_session_id = thread.read(cx).parent_session_id().cloned();
        let visible_history_entries = (0..thread.read(cx).entries().len()).collect();
        ...
        let mut this = Self {
            ...
            permission_text_editors: RefCell::new(HashMap::default()),
            permission_text_editor_subscriptions: RefCell::new(HashMap::default()),
            _cancel_task: None,
            _save_task: None,
        ...
        this.sync_pending_permission_text_prompt_editors(window, cx);
    ...
    pub fn authorize_tool_call(
        ...
        self.clear_permission_text_prompt_editor_for_session(&session_id, &tool_call_id, cx);
    ...
    pub fn authorize_pending_tool_call(
        ...
        let session_id = self.thread.read(cx).session_id().clone();
        let (target_session_id, tool_call_id, _) = self
            .conversation
            .read(cx)
        ...
        let extra_meta =
            self.permission_text_prompt_value_meta(&target_session_id, &tool_call_id, is_allow, cx);
        self.conversation.update(cx, |conversation, cx| {
            conversation.authorize_pending_tool_call(&session_id, kind, extra_meta, cx)
        })?;
        self.clear_permission_text_prompt_editor_for_session(&target_session_id, &tool_call_id, cx);
        if self.should_be_following {
            self.workspace
                .update(cx, |workspace, cx| {
                    workspace.follow(CollaboratorId::Agent, window, cx);
    ...
    fn handle_authorize_tool_call(
        ...
        let current_session_id = self.thread.read(cx).session_id().clone();
        let session_id = self
            .conversation
            .read(cx)
            .session_id_for_tool_call(&current_session_id, &tool_call_id, cx)
        ...
        let extra_meta =
            self.permission_text_prompt_value_meta(&session_id, &tool_call_id, is_allow, cx);
        let outcome = self.selected_permission_outcome_for_action(
            &session_id,
            &tool_call_id,
            &option_id,
            option_kind,
    ...
    #[cfg(test)]
    pub(crate) fn set_permission_text_prompt_value_for_test(
        &mut self,
        tool_call_id: &acp::ToolCallId,
        value: &str,
        window: &mut Window,
        ...
        let editor = self
            .permission_text_editors
            .borrow()
            .get(tool_call_id)
            .cloned()
            .expect("permission text prompt editor should exist");
        editor.update(cx, |editor, cx| {
    ...
    pub(crate) fn clear_permission_text_prompt_editor(&mut self, tool_call_id: &acp::ToolCallId) {
        self.permission_text_editors
            .borrow_mut()
            .remove(tool_call_id);
        self.permission_text_editor_subscriptions
            .borrow_mut()
            .remove(tool_call_id);
    }

    fn clear_permission_text_prompt_editor_for_session(
        &mut self,
        session_id: &acp::SessionId,
        tool_call_id: &acp::ToolCallId,
        cx: &mut Context<Self>,
        ...
        } else if let Some(thread_view) = self.thread_view_for_session(session_id, cx) {
            thread_view.update(cx, |thread_view, _cx| {
                thread_view.clear_permission_text_prompt_editor(tool_call_id);
            });
        }
    }

    fn selected_permission_outcome_for_action(
        &self,
        session_id: &acp::SessionId,
        tool_call_id: &acp::ToolCallId,
        ...
        let outcome = self
            .thread_for_session(session_id, cx)
            .and_then(|thread| {
                thread
                    .read(cx)
                    ...
                    .and_then(|(_, tool_call)| {
                        ...
                        self.selected_permission_outcome_from_options(
                            options,
                            tool_call_id,
                            option_id,
                            is_allow,
            ...
            .unwrap_or_else(|| SelectedPermissionOutcome::new(option_id.clone(), option_kind));

        merge_permission_outcome_meta(outcome, extra_meta)
    }
    ...
    fn thread_for_session(
        ...
        if &self.session_id == session_id {
            ...
            self.thread_view_for_session(session_id, cx)
                .map(|thread_view| thread_view.read(cx).thread.clone())
        }
    }

    fn permission_text_editor_for_session(
        &self,
        session_id: &acp::SessionId,
        tool_call_id: &acp::ToolCallId,
        cx: &App,
    ...
    fn permission_text_prompt_for_session(
        &self,
        session_id: &acp::SessionId,
        tool_call_id: &acp::ToolCallId,
        cx: &App,
    ) -> Option<PermissionTextPrompt> {
        self.thread_for_session(session_id, cx).and_then(|thread| {
            thread
                .read(cx)
                .tool_call(tool_call_id)
    ...
    fn selected_permission_outcome_from_options(
        &self,
        ...
        if let Some(PermissionSelection::SelectedPatterns(checked)) =
            self.permission_selections.get(tool_call_id)
            && let Some(outcome) = options.build_outcome_for_checked_patterns(checked, is_allow)
        {
            return Some(outcome);
        }
        ...
        match options {
            ...
            | PermissionOptions::DropdownWithPatterns { choices, .. } => choices
                ...
                .find(|choice| {
                    if is_allow {
                        &choice.allow.option_id == option_id
                    } else {
                        &choice.deny.option_id == option_id
                    }
                })
    ...
    fn permission_text_prompt_allows_authorization(
        ...
        let Some(prompt) = self.permission_text_prompt_for_session(session_id, tool_call_id, cx)
        else {
            return true;
        };
        ...
        self.permission_text_editor_for_session(session_id, tool_call_id, cx)
            .map(|editor| editor.read(cx).text(cx))
            .as_deref()
    ...
    fn permission_text_prompt_value_meta(
        ...
        let editor = self.permission_text_editor_for_session(session_id, tool_call_id, cx)?;
        Some(acp::Meta::from_iter([(
            TEXT_PROMPT_VALUE_META_KEY.into(),
            editor.read(cx).text(cx).into(),
        )]))
    }
    ...
    fn collect_subagent_items_for_sessions(
        ...
        awaiting_session_ids
            ...
            .filter_map(|session_id| {
                let (summary, entry_ix) = tool_calls_by_session.get(session_id)?;
                let visible_ix = visible_history_entries
                    .iter()
                    .position(|visible_entry_ix| visible_entry_ix == entry_ix)?;
                Some((summary.clone(), visible_ix))
            })
            .collect()
    }
    ...
    fn render_subagents_awaiting_permission(&self, cx: &Context<Self>) -> Option<AnyElement> {
        ...
        let subagent_items = Self::collect_subagent_items_for_sessions(
            entries,
                ...
                .child(
                    v_flex().children(subagent_items.into_iter().enumerate().map(
                        |(ix, (label, visible_ix))| {
                            ...
                            let group = format!("group-{}", visible_ix);

                            h_flex()
                                .cursor_pointer()
                                .id(format!("subagent-permission-{}", visible_ix))
                                .group(&group)
                                .p_1()
                                .pl_2()
                                .min_w_0()
    ...
    fn entry_is_below_viewport(&self, visible_ix: usize) -> bool {
        let viewport_bounds = self.list_state.viewport_bounds();
        self.list_state
            .bounds_for_item(visible_ix)
    ...
    pub(crate) fn render_main_agent_awaiting_permission(
        ...
        let visible_ix = self.visible_index_for_entry(entry_ix)?;

        if !self.entry_is_below_viewport(visible_ix) {
            return None;
        }
...
impl ThreadView {
    #[cfg(test)]
    pub(crate) fn set_history_filter_enabled_for_test(
        &mut self,
        ...
        match kind {
            HistoryFilterKind::UserMessages => self.history_filters.user_messages = enabled,
            HistoryFilterKind::AssistantMessages => {
                self.history_filters.assistant_messages = enabled
            }
            HistoryFilterKind::Thinking => self.history_filters.thinking = enabled,
            HistoryFilterKind::ToolCalls => self.history_filters.tool_calls = enabled,
            HistoryFilterKind::ToolResults => self.history_filters.tool_results = enabled,
            HistoryFilterKind::PermissionRequests => {
                self.history_filters.permission_requests = enabled
            }
            HistoryFilterKind::ErrorsAndCancels => {
                self.history_filters.errors_and_cancels = enabled
            }
        }
    ...
    #[cfg(test)]
    pub(crate) fn visible_history_entries_for_test(&self) -> &[usize] {
        &self.visible_history_entries
    }
    ...
    pub(crate) fn scroll_to_most_recent_user_prompt(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(ix) = entries
            ...
            .and_then(|entry_ix| self.visible_index_for_entry(entry_ix))
        {
    ...
    fn scroll_output_to_previous_message(
        ...
        let visible_entries = &self.visible_history_entries;
        if let Some(target_ix) = visible_entries[..current_ix.min(visible_entries.len())]
            .iter()
            .rev()
            .copied()
    ...
    fn scroll_output_to_next_message(
        ...
        let current_ix = self.list_state.logical_scroll_top().item_ix;
        let visible_entries = &self.visible_history_entries;
        if let Some(target_ix) = visible_entries
            .iter()
            .enumerate()
            .skip(current_ix.saturating_add(1))
            .find_map(|(visible_ix, &entry_ix)| {
                matches!(
    ...
    pub(crate) fn sync_history_filter_projection(&mut self, cx: &App) {
        let entries = self.thread.read(cx).entries();
        let next_visible_entries = entries
            .iter()
            .enumerate()
            .filter_map(|(ix, entry)| self.history_filters.allows_entry(entry).then_some(ix))
            .collect();
        self.set_visible_history_entries(next_visible_entries, cx);
    }

    fn visible_index_for_entry(&self, entry_ix: usize) -> Option<usize> {
        self.visible_history_entries
            .iter()
            .position(|visible_entry_ix| *visible_entry_ix == entry_ix)
    }

    fn focus_handles_for_entries(
        &self,
        entry_indexes: &[usize],
        cx: &App,
    ...
    fn set_visible_history_entries(&mut self, next_visible_entries: Vec<usize>, cx: &App) {
        if self.visible_history_entries == next_visible_entries {
            return;
        }

        let mut unchanged_prefix_len = 0;
        while unchanged_prefix_len < self.visible_history_entries.len()
            && unchanged_prefix_len < next_visible_entries.len()
            && self.visible_history_entries[unchanged_prefix_len]
                == next_visible_entries[unchanged_prefix_len]
        {
            unchanged_prefix_len += 1;
        }

        let mut old_suffix_start = self.visible_history_entries.len();
        let mut new_suffix_start = next_visible_entries.len();
        while old_suffix_start > unchanged_prefix_len
            && new_suffix_start > unchanged_prefix_len
            && self.visible_history_entries[old_suffix_start - 1]
                == next_visible_entries[new_suffix_start - 1]
        {
            old_suffix_start -= 1;
            new_suffix_start -= 1;
        }

        let focus_handles = self.focus_handles_for_entries(
            &next_visible_entries[unchanged_prefix_len..new_suffix_start],
            cx,
        );
        self.list_state
            .splice_focusable(unchanged_prefix_len..old_suffix_start, focus_handles);
        self.visible_history_entries = next_visible_entries;
    }
...
;
    ...
    fn render_permission_buttons(
        ...
        entry_ix: usize,
        tool_call_id: acp::ToolCallId,
        tool_call_meta: Option<&acp::Meta>,
        focus_handle: &FocusHandle,
        cx: &Context<Self>,
    ) -> Div {
        let text_prompt = PermissionTextPrompt::from_meta(tool_call_meta);
        ...
        v_flex()
            .when(text_prompt.is_some(), |this| {
                this.child(self.render_permission_text_prompt_editor(&tool_call_id, cx))
            })
            .child(buttons)
    }

    fn render_permission_text_prompt_editor(
        &self,
        tool_call_id: &acp::ToolCallId,
        cx: &Context<Self>,
    ) -> Div {
        let editor = self
            ...
            .get(tool_call_id)
            .cloned();
        div().when_some(editor, |this, editor| {
            this.p_2()
                .border_t_1()
                .border_color(self.tool_card_border_color(cx))
                .child(
    ...
    pub(crate) fn sync_permission_text_prompt_editor(
        &mut self,
        tool_call_id: &acp::ToolCallId,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        let editor = cx.new(|cx| {
            let mut editor = if prompt.is_multiline() {
        ...
        let subscription = cx.observe(&editor_for_subscription, |_this, _editor, cx| cx.notify());
        self.permission_text_editor_subscriptions
            .borrow_mut()
            .insert(tool_call_id.clone(), subscription);
    }

    fn sync_pending_permission_text_prompt_editors(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let pending_prompt_ids = self
            .thread
            .read(cx)
            .entries()
            .iter()
            .filter_map(|entry| {
                ...
                PermissionTextPrompt::from_meta(tool_call.meta.as_ref())?;
                Some(tool_call.id.clone())
            })
            .collect::<Vec<_>>();

        for tool_call_id in pending_prompt_ids {
            self.sync_permission_text_prompt_editor(&tool_call_id, window, cx);
        }
    }
    ...
    fn render_permission_buttons_with_dropdown(
        ...
        session_id: acp::SessionId,
        tool_call_id: acp::ToolCallId,
        text_prompt: Option<&PermissionTextPrompt>,
        focus_handle: &FocusHandle,
        cx: &Context<Self>,
    ) -> Div {
        ...
        let text_editor = text_prompt.and_then(|_| {
            self.permission_text_editors
                .borrow()
                .get(&tool_call_id)
                .cloned()
        });
        let allow_disabled_for_required_text = text_prompt.is_some_and(|prompt| {
            prompt.required
                && text_editor
                    .as_ref()
                    .map(|editor| editor.read(cx).text(cx))
                ...
                h_flex()
                    ...
                    .child(
                        Button::new(("allow-btn", entry_ix), "Allow")
                            ...
                            .disabled(allow_disabled_for_required_text)
                            .on_click(cx.listener({
